<template>
  ...
  <v-dialog v-else-if="!user" v-model="dialog" max-width="800" :fullscreen="$vuetify.breakpoint.xs">
    <template v-slot:activator="{ on, attrs }">
      <v-btn icon dark v-bind="attrs" v-on="on"><v-icon>mdi-account</v-icon></v-btn>
    </template>
    ...
    <v-card>
      <v-list>
        <v-list-item>
          ...
          <v-list-item-content>
            <v-list-item-title>Pen Name</v-list-item-title>
      ...
      <v-card-actions>
        <v-spacer/>
        <v-btn color="accent" dark @click="signOut">SignOut</v-btn>
      </v-card-actions>
    </v-card>
  </v-menu>
</template>
