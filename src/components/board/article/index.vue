...
export default {
  ...
  methods: {
    ...
    subscribe (arrow) {
      ...
      if (!this.category) {
        this.ref = this.$firebase.firestore()
          ...
          .collection('articles').orderBy('important', 'desc').orderBy(this.order, this.sort).limit(LIMIT)
      } else {
        this.ref = this.$firebase.firestore()
          .collection('boards').doc(this.boardId)
          .collection('articles')
          .where('category', '==', this.category)
          .orderBy('important', 'desc').orderBy(this.order, this.sort).limit(LIMIT)
      }
      this.loaded = false
    ...
    onViewerLoad (v) {
      addYoutubeIrame(v.preview.el, this.$vuetify.breakpoint)
    }
  }
}
</script>
